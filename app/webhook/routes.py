...
def receiver():
    ...
    actions_data.insert({"request_id": data['commits'][0]['id'], "author": data['head_commit']['author']['username'], "action": "PUSH", "from_branch": data['ref'].split("/")[-1], "to_branch": data['ref'].split("/")[-1], "timestamp": data['repository']['updated_at']})
...
def action_update():
    ...
    data = actions_data.find().sort("_id",-1).limit(count)
    # self.count = 0
    line = list()
    for i in data:
